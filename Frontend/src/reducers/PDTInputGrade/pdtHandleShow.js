...
const reducer = (state = initialState, action) => {
  switch (action.type) {
    case pdtConstants.PDT_HANDLE_SHOW: {
      ...
      if (data.titleSchoolYear) state.titleSchoolYear = data.titleSchoolYear;
      if (data.Year) state.Year = data.Year;
      if (data.Semester) state.Semester = data.Semester;
      if (data.titleDepartment) state.titleDepartment = data.titleDepartment;
      ...
      if (data.stt) state.stt = data.stt;
